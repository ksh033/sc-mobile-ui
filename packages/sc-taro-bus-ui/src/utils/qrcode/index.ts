...
import { getPixelRatio } from '../common';
...
function drawQrcode(options, debug = false) {
  options = options || {};
  ...
  if (debug) {
    var innerQrcode = new QRCode(options.typeNumber, options.correctLevel);
    innerQrcode.addData(utf16to8(options.text));
    innerQrcode.make();
  ...
  function createCanvas() {
    ...
    const ctx = Taro.createCanvasContext(options.canvasId);
    // todo

  ...
  (props, ref) => {
    ...
    const _render = (rlist: any, callback: any) => {
      ...
      for (let i = 0; i < rlist.length; i++) {
        ...
        //   : [];
        const colsLen =
          Array.isArray(newCols) && newCols.length > 0
            ? newCols.map(it => it.length)
            : [0, 0]
        // let minHeightIndex = colsHeight.indexOf(Math.min.apply(Math, colsHeight))
        let minHeightIndex = colsLen.indexOf(Math.min.apply(Math, colsLen))
        let len =
          newCols[minHeightIndex] != null &&

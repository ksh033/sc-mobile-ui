...
import { strictOf } from '../utils/helper';
...
export function stringfiyUrl<R extends RecordData = RecordData>(
  ...
  let stringfiyUrlStr = url;
  if (options && strictOf<R>(options, 'Object')) {
    const hasQuery = stringfiyUrlStr.includes('?');
    stringfiyUrlStr += (hasQuery ? '&' : '?') + stringify(options);
  }
  return stringfiyUrlStr;
}
